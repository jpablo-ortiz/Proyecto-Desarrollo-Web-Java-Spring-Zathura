...
public class PlanetaControllerIntegrationTest {
    ...
    public void end() {
        if (finalizado) {
            ...
            this.planetaXProductoRepository.deleteAll();
    ...
    void obtenerPlaneta() {
        Planeta planeta = rest.withBasicAuth("usuario1", "1234")
                .getForObject("http://localhost:" + port + "/planeta/11", Planeta.class);
        assertEquals(11, planeta.getId());
    }

    @Test
    void obtenerPlanetas() {
        List<Planeta> planetas = rest.withBasicAuth("usuario1", "1234")
                .getForObject("http://localhost:" + port + "/planeta", ArrayList.class);
    ...
    void obtenerPlanetasPorEstrella() {
        List<Planeta> planetas = rest.withBasicAuth("usuario1", "1234")
                .getForObject("http://localhost:" + port + "/planeta/estrella/1", ArrayList.class);
        assertEquals(2, planetas.size());
    }

    @Test
    void obtenerProductosPorPlaneta() {
        List<Producto> productos = rest.withBasicAuth("usuario1", "1234")
                .getForObject("http://localhost:" + port + "/planeta/productos/11", ArrayList.class);
        assertEquals(1, productos.size());
    }

    @Test
    void obtenerPlanetaXProducto() {
        PlanetaXProducto pxp = rest.withBasicAuth("usuario1", "1234")
                .getForObject("http://localhost:" + port + "/planeta/11/producto/16", PlanetaXProducto.class);
